                  ...
                  n.scales &&
                    ...
                    a.each(r, function (n) {
                      ...
                      if (l in o && o[l].type === u)
                        ((d = o[l]).options = r), (d.ctx = i.ctx), (d.chart = i)
                      else {
...
;(function () {
  ...
  "undefined" === typeof Chart
    ...
    : ((c.prototype.beforeDatasetsUpdate = function (a) {
      ...
      (c.prototype.drawDataset = function (a) {
        for (
          var b = this.ctx,
            p = this.chartInstance,
            l = a._meta[Object.keys(a._meta)[0]],
            h = 0,
            f = 0;
          f < l.data.length;
          f++
          ...
          if (0 !== d.circumference || this.showZero) {
            ...
            b.save()
            b.beginPath()
            b.font = Chart.helpers.fontString(
              this.fontSize,
              this.fontStyle,
              this.fontFamily
            )
            if (
              ...
              ("border" === this.position && "pie" === p.config.type)
            ) {
              ...
              "border" === this.position
                ? (c = (d.outerRadius - k) / 2 + k)
                : "outside" === this.position && (c = d.outerRadius - k + k + m)
              n = { x: d.x + Math.cos(n) * c, y: d.y + Math.sin(n) * c }
              if ("outside" === this.position) {
                n.x = n.x < d.x ? n.x - m : n.x + m
                var r = d.outerRadius + m
              }
            } else (k = d.innerRadius), (n = g.tooltipPosition())
            m = this.fontColor
            "function" === typeof m
              ? (m = m({
                  label: p.config.data.labels[f],
              ...
              : "string" !== typeof m &&
                (m = m[f] || this.options.defaultFontColor)
            if (this.arc)
      ...
      (c.prototype.checkTextBound = function (a, b, p, l) {
        for (var h = this.labelBounds, f = 0; f < h.length; ++f) {
          for (
            var g = h[f],
              d = [
                [a, p],
            ...
            var c = d[e][0],
              k = d[e][1]
            if (c >= g.left && c <= g.right && k >= g.top && k <= g.bottom)
              return !1
          }
          d = [
            [g.left, g.top],
        ...
        h.push({ left: a, right: b, top: p, bottom: l })
        return !0
      }),
      ...
      (c.prototype.drawArcText = function (a, b, c, l) {
        var h = this.ctx,
          f = c.x,
          g = c.y,
          d = c.startAngle
        ...
        if (l || !(c - d > g))
          if ("string" === typeof a)
            for (h.rotate(d), l = 0; l < a.length; l++)
              (d = a.charAt(l)),
                (f = h.measureText(d)),
                h.save(),
                h.translate(0, -1 * b),
